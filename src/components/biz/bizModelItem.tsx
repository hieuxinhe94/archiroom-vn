import { CircularProgress, Input, Progress, Select, SelectItem, Skeleton } from '@nextui-org/react'
...
var uploadCount = 0;
export default function BizModelItem(props) {
    const originModel = (props?.item as BizModel);
    const session = props?.session;
    let currentUser: UserEntity = props?.currentUser ?? getUpdateAuthenticateUser({});

    const [currentModel, setCurrentModel] = useState<BizModel>({
        bizId:   currentUser.bizId ??  session?.user?.email ?? '000',
        name: originModel?.name ?? 'Demo Model',
        images: originModel?.images ?? [],
        id: originModel?.id ?? null
    });
  
    const router = useRouter()
    const [selectedImages, setSelectedImages] = useState<File[]>(null);
    const [uploadedImages, setUploadedImages] = useState<string[]>(originModel?.images);
    const [uploadImageFileCount, setUploadImageFileCount] = useState<number>(0);
    const [modelName, setModelName] = useState(originModel?.name);
    const [modelDesc, setModelDesc] = useState(originModel?.desc);
    const [modelAge, setModelAge] = useState(originModel?.age?.toString());
    ...
    const onCreateModel = useCallback(async () => {
        ...
        currentModel.images =  uploadedImages;
        if (!currentModel.images) {
            currentModel.images = []
        }

        var tempUploadedImages = currentModel.images;
        
        
        // wait for upload images:
        selectedImages?.forEach(async (image, i) => {
            var newFileName = `${currentUser?.id ?? 'anonymous'}-${encodeURI(modelName)}-${i}` + '.png'
            ...
            await imageKitService
                ...
                .then((uploaded) => {
                    ...
                    tempUploadedImages.push(uploaded.url + "?tr=w-768,h-1024")
                    currentModel.images = tempUploadedImages;
        ...
        currentModel.desc = modelDesc;
        currentModel.age = Number.parseInt(modelAge);

        await ensureCallAPIVTO().then(async function () {
            await vtoService.createNewModel(currentModel).then(
                (res) => {
                    
                    router.reload()
                    //BizModels.push(currentModel)
    ...
    }, [selectedImages, currentUser, currentModel, ensureCallAPIVTO, modelAge, modelName, modelDesc, router, uploadedImages, session]);
    ...
    const onDeleteModel = useCallback(async () => {
        ...
        await vtoService.deleteModel(currentModel.id).then(
            (res) => {
    ...
    }, [currentModel, router, currentUser.username, session]);
                        ...
                        <div className="flex h-[90%] w-full flex-col gap-4 rounded-medium border-small border-divider  p-8">
                            ...
                            <div className="flex flex-col gap-4 max-w-sm">
                                <Input
                                    ...
                                    value={modelDesc}
                                    onValueChange={setModelDesc}
                                    isInvalid={(modelDesc?.length <= 3)}
                                    type="text"
                                    ...
                                    <div className="group  gap-2 px-4 pt-4">
                                        {
                                            ...
                                            (<div key={i} className="relative inline-flex shrink-0">
                                                <div   className="relative shadow-black/5 shadow-none rounded-large" style={{ maxWidth: "fit-content;" }}>
                                                    <img
                                                        onClick={() => props.onPreViewItem(uploadedImages, i)}
                                                        src={url}
                                                ...
                                                <div data-slot="inner-wrapper" className="inline-flex w-full h-full box-border items-start group-data-[has-label=true]:items-start pb-12 pt-2 relative">
                                                    ...
                                                    <div className="absolute right-0 flex h-full flex-col items-end justify-between gap-2">
                                                        ...
                                                        <div className="flex items-end gap-2">
                                                            <p className="py-1 text-tiny text-default-400">{uploadImageFileCount}/max 5 image uploaded</p>
